...
The backend supports the core LMS modules for authentication, courses, assessments, timetable management, communication, resources, AI integration, and the **Parent QR / Student Report** workflow.

The Parent Report endpoint is designed to work without a parent account because access is initiated from the student's QR-generated report URL.
...
- **Parent Report API**: Public endpoint for QR-based student report access
- **Student Information**: Name, email, and registration number
- **Course Information**: Enrolled courses with teacher details
- **Assessment Results**: Test marks, percentage, and grading status
- **QR-Based Access**: Student dashboard generates a report URL that can be encoded as a QR code
- **No Parent Login Required**: Report is accessed directly through the QR-generated URL
...
│   ├── studentController.js   # Student + parent report operations
...
This endpoint is intentionally placed before the authenticated student routes so that the report can be opened directly after scanning the Parent QR.
...
GET /api/student/parent-report/:studentId
       ↓
Backend fetches student + courses + submissions
       ↓
Standalone Student Report
```
...
3. Confirm the Parent QR is visible.
4. Scan the QR using a phone.
5. Confirm the standalone Parent Report opens.
6. Verify student details.
7. Verify enrolled courses when available.
8. Verify test results when submissions exist.

For real phone testing, use the deployed frontend URL. A QR generated from a local `localhost` URL will normally not open the computer's local server from another phone.
...
The frontend uses `VITE_API_BASE_URL` to communicate with the deployed backend API.
...
[![GitHub stars](https://img.shields.io/github/stars/bs-bhaskar/lms-project?style=social)](https://github.com/bs-bhaskar/smart-lms/stargazers)
